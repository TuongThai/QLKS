...
public class LoginController {
    ...
    public ModelAndView loginRequestHandler(ModelMap modelMap, HttpSession session, HttpServletRequest request) {
        EmployeeModel employee = loginService.checkCookie(request);//check xem nhân viên đó có khôgn
        if (employee == null) {
	...
	public ModelAndView login(ModelMap model,@RequestParam("taiKhoan") String taiKhoan,
		...
		if (employee == null) {
			model.addAttribute("message", "Tài khoản hoặc mật khẩu không đúng.");// thêm thuộc tính
